...
**z 边界只有一种**：x/y 周期，z 上下无滑移硬壁（`boundary_z = noslip`，也是默认值）。
...
`dump_config` 会把 **`h_rest` 与 `z_rest`** 一并打印 —— 这是运维时唯一能一眼看出
「壁面势配软了（粒子压穿）/ 配硬了（粒子悬在半空）」的数。27 个键里只有 `wall_*`
的 8 个是新增的，与 `pot_*` 走**同一份**查表/校验 helper。
