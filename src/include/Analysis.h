...
// 能量均分目标值
//
//   <|V|^2> = 2kT/M_i - 2kT/(rho L^3)
//
// 第二项是 k=0 模式被冻结在零的修正。代码中 Sum_r v_alpha 逐步严格守恒于 0：
// 压力在 k=0 被清零（Stokes.cpp）、div(Pi) 在周期域上是望远镜求和恒为零、
// 且本测试中 Sum f = Sum F = 0。
//
// 【为什么是 2 个自由度而不是 3 个】基线式 2kT/M_i 来自 Parseval 对【全部】k
// 求和，每个 k 的权重是 tr P(k) = 2（横向投影算子的迹，与 k 无关）—— 也就是说
// k=0 已按权重 2 被计入。代码把它压到 0，故减 2 个自由度。
// （真实平衡态 k=0 有 3 个自由度不受不可压约束，是 2kT/M_i + kT/(rho L^3)。）
// ---------------------------------------------------------------------------
