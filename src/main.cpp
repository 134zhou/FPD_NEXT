...
static int run_noise_check(int L, double dt, double kT, long n_steps)
{
    ...
    for (long step = 0; step < n_steps; step++)
    {
        ...
        step_navier_stokes(cfg, d_vx, d_vy, d_vz, d_p, d_fx, d_fy, d_fz,
                           ...
                           d_fft, plan, gen, d_rD, d_rN, d_tfx, d_tfy, d_tfz, step);
...
static int run_production(const FpdConfig& c)
{
    ...
    std::vector<double> vx(size, 0.0), vy(size, 0.0), vz(size, 0.0), p(size, 0.0);
    std::vector<double> fx(size, 0.0), fy(size, 0.0), fz(size, 0.0);
    std::vector<double> Rx(N, 0.0), Ry(N, 0.0), Rz(N, 0.0);
    std::vector<double> Rux(N, 0.0), Ruy(N, 0.0), Ruz(N, 0.0);
    std::vector<double> Vx(N, 0.0), Vy(N, 0.0), Vz(N, 0.0);
    std::vector<double> Fx(N, 0.0), Fy(N, 0.0), Fz(N, 0.0);
    ...
    CkptArrays arr;
    arr.vx = vx.data(); arr.vy = vy.data(); arr.vz = vz.data();
    arr.p  = p.data();
    arr.Rx = Rx.data(); arr.Ry = Ry.data(); arr.Rz = Rz.data();
    arr.Rux = Rux.data(); arr.Ruy = Ruy.data(); arr.Ruz = Ruz.data();
    arr.Vx = Vx.data(); arr.Vy = Vy.data(); arr.Vz = Vz.data();
    arr.Fx = Fx.data(); arr.Fy = Fy.data(); arr.Fz = Fz.data();
    ...
    {
        // 内置默认：单粒子放盒心的一般亚格点位置
        Rx[0] = cfg.Nx / 2.0 + 0.3; Ry[0] = cfg.Ny / 2.0 + 0.7; Rz[0] = cfg.Nz / 2.0 + 0.5;
        Rux[0] = Rx[0]; Ruy[0] = Ry[0]; Ruz[0] = Rz[0];
    ...
    double* d_Vx = Vx.data(); double* d_Vy = Vy.data(); double* d_Vz = Vz.data();
    double* d_Fx = Fx.data(); double* d_Fy = Fy.data(); double* d_Fz = Fz.data();
    double* d_spx = sum_phix.data(); double* d_spy = sum_phiy.data(); double* d_spz = sum_phiz.data();
    double* d_eta = eta.data(); double* d_etaXY = etaXY.data();
    double* d_etaYZ = etaYZ.data(); double* d_etaZX = etaZX.data();
    double* d_fft = fft_data.data();
    double* tmp_fx = h_tmp_fx.data(); double* tmp_fy = h_tmp_fy.data(); double* tmp_fz = h_tmp_fz.data();
    ...
    // 【逐位重启的关键】必须用 Philox：实测只有它的 setGeneratorOffset 计数单位
    // 与 curandGenerateNormalDouble 产出的 double 个数 1:1 对应。
    // XORWOW（原来的 CURAND_RNG_PSEUDO_DEFAULT）和 MRG32K3A 都对不上。
    // 证据见 spike/spike_rng_offset.cpp。
    curandGenerator_t gen;
    CURAND_CHECK(curandCreateGenerator(&gen, CURAND_RNG_PSEUDO_PHILOX4_32_10));
    CURAND_CHECK(curandSetPseudoRandomGeneratorSeed(gen, h.seed));
    // 不在这里设 offset —— step_navier_stokes 每步都按 step 显式定位到自己的 slot，
    // 所以重启无需恢复任何 RNG 内部状态，只需 seed 相同。
    ...
    // 只做文件 IO，不含任何 acc pragma。
    // ⚠️ acc update 不能放在 lambda 里 —— 闭包捕获让运行时查不到设备映射，
    //    会在运行时报 "data in update host clause was not found on device"。
    struct CkptWriter
    {
        const FpdConfig& c;
        const CkptHeader& h;
        const CkptArrays& arr;
    ...
    const CkptWriter writer{c, h, arr};
    ...
    for (long step = start_step; step < c.n_steps; step++)
    {
        if (step % c.interval_ckpt == 0)
        {
            ...
            #pragma acc update host(d_Rx[0:N], d_Ry[0:N], d_Rz[0:N], d_Rux[0:N], d_Ruy[0:N], d_Ruz[0:N])
            // Fx/Fy/Fz 也拷 —— 原来漏了，现在力恒为 0 所以无害，
            // Phase 5 加粒子间力后会写出陈旧的主机端零值
            #pragma acc update host(d_Vx[0:N], d_Vy[0:N], d_Vz[0:N], d_Fx[0:N], d_Fy[0:N], d_Fz[0:N])
            if (!writer.write(step)) { rc = 2; break; }
        }

        if (step % c.interval_log == 0)
        {
            #pragma acc update host(d_vx[0:size], d_Rx[0:N], d_Ry[0:N], d_Rz[0:N])
            bool bad = false;
            for (int i = 0; i < size; i++)
            { if (!std::isfinite(vx[i])) { bad = true; break; } }
            if (!std::isfinite(Rx[0])) { bad = true; }
            ...
            if (bad)
            {
                #pragma acc update host(d_vy[0:size], d_vz[0:size], d_p[0:size])
                #pragma acc update host(d_Rux[0:N], d_Ruy[0:N], d_Ruz[0:N])
                #pragma acc update host(d_Vx[0:N], d_Vy[0:N], d_Vz[0:N], d_Fx[0:N], d_Fy[0:N], d_Fz[0:N])
                writer.write(step);
                rc = 3;
                break;
        ...
        update_viscosity_fields(cfg, pp, N, d_Rx, d_Ry, d_Rz,
                                d_spx, d_spy, d_spz, d_eta, d_etaXY, d_etaYZ, d_etaZX);
        update_force_field(cfg, pp, N, d_Rx, d_Ry, d_Rz, d_Fx, d_Fy, d_Fz,
                           d_spx, d_spy, d_spz, d_fx, d_fy, d_fz);
        step_navier_stokes(cfg, d_vx, d_vy, d_vz, d_p, d_fx, d_fy, d_fz,
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "用法: fpd <配置文件> [--set key=value ...]\n"
                  << "      fpd --check | --lambda [L] | --noise ... | --equipart ...\n\n";
        print_config_help();
        return 2;
    }
    ...
    const std::string used = fc.out_dir + "/config.used";
