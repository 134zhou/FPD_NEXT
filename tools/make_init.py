...
>= range（相场截断半径）的区间内，越界的构型会直接报错退出。
"""
...
def main():
    ...
    # 粒子必须离两壁 >= range，否则相场支撑域被截断、初始构型就已在物理上无效。
    # 默认 range 与 C++ 侧 make_phi_params 一致：ceil(radius + 3*xi) + 1。
    # z 向无滑移壁面是【唯一】的 z 边界，所以这个边距是无条件的。
    ...
    if a.single:
        ...
        pts = place_random(Nx, Ny, Nz, a.random, min_sep, a.seed, z_margin=z_margin)
