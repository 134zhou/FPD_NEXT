...
单粒子（`wall_eps=1, wall_sigma=2, gravity_z=-10`，32³，200k 步）：
粒子从盒心沉降后停在 **z ≈ 4.68~4.73**，与独立解出的 `z_rest = 4.7189` 吻合。

多粒子集体沉降：`config/sed.cfg`，128×64×32，N=95（标称 φ=0.0497）。
吞吐实测 **116 步/秒**（对比 N=1 的 153 —— 粒子开销约 24%，来自
`update_viscosity_fields` / `update_force_field` 的 N × n_range³ 次原子散写）。
