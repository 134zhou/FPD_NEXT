...
第二阶段已完成：Config.cpp 从 674 行减至 405 行（减少约 40%），AGENTS.md 从
319 行减至 114 行。配置文件和 --set 共用查找赋值逻辑；重复键最后生效；归档
全部字段；main 删除重复的输出目录创建。基础必填项的读取时机保持不变，
所选势必填项仍在 --set 后检查；不改数值内核、壁面力或 .fpd 格式。
新增纯 CPU `--check-config`，覆盖缺项、覆盖顺序、解析错误、两类势参数和归档往返。
同步修正 README 中误把非周期 Nz 纳入截断上界的旧说明：当前上界为 min(Nx,Ny)/2。

验收详见 `baseline/simplify_20260920/RESULTS.md`：配置回归 26 PASS；
--check / --check-potential / --check-poisson / --check-wall 共 64 PASS，
四份日志前后逐字节相同。GPU 路径显式指定 ACC_DEVICE_TYPE=nvidia，未采信沙箱内
可能的 CPU 回退。两种单粒子配置（无壁面势、近壁 WCA）各 5 个检查点前后整文件
逐位相同；两种配置的连续 200 步与 100+100 重启也整文件逐位相同。
冻结程序和 .fpd 留在本机，日志、配置、校验和与复现脚本归档入库。

## `.fpd` v3：文件只保存续跑状态（2026-09-21）

用户确认每次运行都有 init_file。v3 文件头只保留 magic/endian/version、Nx/Ny/Nz/N、step；
数据区保留速度和粒子数组，删除压力数组、flags、物理量及 RNG 记录；末尾 FNV 校验保留。
压力仍用于泊松求解与数值判据，不再作为检查点或可视化产物输出。v2 明确拒绝，
旧版文件继续由冻结的 v2 程序读取，本次没有迁移工具。

配置或 --set 显式出现 Nx/Ny/Nz 即报错；init_file、n_steps 和物理基础参数必填。
生产路径使用同一句柄读头、分配数组并验证载荷，seed、dt 等来自配置。
离线转换和沉降统计通过显式 --config 获取物理参数；.config.used 不输出尺寸键。

验证见 `baseline/io_v3_20260921/RESULTS.md`：四项数值判据共 64 PASS 且日志与
原基线逐字节相同；两种算例 v2/v3 共用载荷在 step 0/50 逐位相同，v3 的
50 步与 25+25 步重启整文件逐位相同。互操作、近壁力复算和 VTK 往返通过。
