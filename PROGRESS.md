...
最后更新：2026-09-11（Phase 7-B 完成，z 向壁面接线）
...
**z 向无滑移壁面（Phase 7）已完成并接线到生产路径**：切向无滑移 ghost、
壁面剪切噪声 ×√2、相场按 `Loc` 截断、棱边数组 Nz+1 层。数学推导见
`doc/PressurePoisson.md`（§14 是壁面 BC 与噪声），端到端判据 W1–W9 全绿。
...
| 6 | 生产算例 + 旧代码对照 | ⬜ 未开始 |
| 7 | z 向无滑移壁面（重力沉降） | ✅ 完成（7-A 求解器 + 7-B 接线，W1–W9 全绿） |
...
**Phase 7 已全部完成**（0/1/2/3/5/7 完成，剩 4 与 6）。下一步建议 **Phase 6
（生产算例 + 旧代码对照）** —— 壁面是重力沉降算例的前提，现在它已经就绪，
可以直接跑「x/y 周期 + z 上下硬壁 + 重力只作用于粒子」的沉降算例。

Phase 4（Stokes 阻力 + VAF/MSD + L 外推）也仍无阻塞项，与 Phase 6 互不阻塞。
原计划的「Phase 4 撤销」判断（FPD 无显式阻力项，`a_eff` 由沉降终端速度顺带测得）
仍然成立 —— 壁面上线后终端速度**更干净**（有真实动量汇，不必靠
`gravity_compensate` 抵消 k=0 漂移）。
...
- **Phase 7-B**（z 向无滑移壁面接线）：✅ **已完成**（见上）。遗留一项：
  本轮**明确不做粒子-壁面排斥势** —— 粒子靠初始构型远离壁面
  （`make_init.py --wall-margin`，离两壁 ≥ `range`），越界报错中止（不 clamp）。
  真正的壁面势是后续阶段的事。
...
9. **W5b 的绝对量测带 ~1.7% 系统偏差**（Phase 7-B 遗留）。壁面能量均分比值
   0.9750±0.0029、周期控制 0.9827±0.0024 —— **两个都不等于 1**，所以这不是壁面
   特有的问题，而是这台 4³ 小盒子量测机器的固有偏差，来源至少有二：`dim` 里「哪些
   模式被冻结」的约定（周期下实测冻结数在 2~3 之间，与 `dim = n_v − size + 1` 的
   纯计数有出入），以及显式 Euler 的 O(dt) 偏差。
   **当前的判据（差分形式）不受影响** —— γ=2 被 8σ 选中这一结论是稳的。
   要做到绝对一致，需要：更大的盒子（减小 `dim` 约定的相对影响）+ dt 外推
   （分离 O(dt)）+ 更长的采样。**未立项。**
