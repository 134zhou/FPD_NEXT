...
void poisson_PBC(double ***data_in, double ***data_out, int Nx, int Ny ,int Nz){
	...
	long status, l[3], strides_out[4]; 
	l[0] = Nx; l[1] = Ny; l[2] = Nz;
	int Nz2 = Nz/2+1;
	strides_out[0] = 0; strides_out[1] = Nz2*Ny;  strides_out[2] = Nz2; strides_out[3] = 1; 
	...
	status = DftiCreateDescriptor( &my_desc_handle, DFTI_DOUBLE, DFTI_REAL, 3, l);
	status = DftiSetValue(my_desc_handle, DFTI_CONJUGATE_EVEN_STORAGE, DFTI_REAL_COMPLEX);
	status = DftiSetValue( my_desc_handle, DFTI_PLACEMENT, DFTI_NOT_INPLACE);
	status = DftiSetValue(my_desc_handle, DFTI_OUTPUT_STRIDES, strides_out);
	status = DftiCommitDescriptor( my_desc_handle);
	status = DftiComputeForward(my_desc_handle, in, out);
	...
	for(int j=0;j<Nx;j++){
		...
		for(int k=0;k<Ny;k++){
			...
			for(int s=0;s<Nz/2;s++){
				if(j==0 && k==0 && s==0){continue;}
				const double cosZm1 = cos(phase_z*s) - 1.;
				out[j][k][s] *= 0.5/(cosXm1 + cosYm1 + cosZm1);
				}
			}
		}
	out[0][0][0] *= 0;
	...
	status = DftiCommitDescriptor( my_desc_handle);
	status = DftiComputeBackward( my_desc_handle, out, in);
