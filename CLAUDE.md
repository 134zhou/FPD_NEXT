...
./build/main --check                          # 自检：力守恒 / 亚格点不变 / N=2 重叠
./build/main --noise [L] [dt] [kT] [steps]    # 测试 3A：纯流体噪声谱，默认 32 0.01 1.0 200000
...
环境：RTX 3060 12 GB。当前吞吐 **153 步/秒**（128×64×32，N=1，GPU 已 99% 饱和）。
排期前先按这个数算机时；Phase 4 的 L=192 立方盒是 27 倍网格量。
...
- **噪声的 O(dt) 偏差**（纯流体 η=1 实测）：`bias(%) ≈ 335 × dt`。
  要 kT 精度 < 1% 需 `dt < 0.0033`。⚠️ 这是 η=1 下测的，
  无量纲群是 `ν·dt/dx²`，粒子内 η=50 使它大 50 倍 —— **粒子侧偏差未知，待测试 3B**
- **平衡时间**：最慢模式 `τ = (L/2π)²/ν`。做平衡态统计时盒子越大越慢，
  纯流体测试务必用小立方盒（L=128 要 41 万步，L=32 只要 2.6 万步）
- **λ^T、M_i 等常量必须用与模拟相同的离散求和**。实测 `∫φ` 比解析球体
  `(4/3)πa³` 大 **24.1%**（a=3.2, ξ=1），用解析公式会毁掉验证
